  ...
  class ArraySerializer
    ...
    class ModelSerializationTest < Minitest::Test
      ...
      def test_associated_objects_of_multiple_instances_embedded_in_root
        ...
        assert_equal({
            posts: [
              {title: "Title 1", body: "Body 1", created_at: nil, updated_at: nil, "comment_ids" => @post1.comments.map(&:object_id) },
              {title: "Title 2", body: "Body 2", created_at: nil, updated_at: nil, "comment_ids" => @post2.comments.map(&:object_id) }
            ],
            comments: [
              {content: "C1"},
              {content: "C2"},
